import os
from abc import ABC  # 用于定义抽象基类（如包含 @abstractmethod 的类）
from enum import Enum  # 创建枚举常量（如定义模型类型 ModelType.GPT2）
from typing import Optional, List  # 类型注解支持  #Optional 表示变量可以是某类型或 None List：声明列表类型（如 scores: List[float]）
import torch
from langchain.callbacks.manager import CallbackManagerForLLMRun   # 回调管理器 处理LLM运行时的回调逻辑（如日志记录、流式输出）
from langchain.llms.base import LLM  # LangChain LLM基类
from transformers import (AutoModel, AutoConfig, AutoTokenizer) # HF模型加载工具
from config import cfg  # 自定义配置文件
...
class ChatGLM_Ptuning(LLM, ABC): # 继承LangChain的LLM基类，实现自定义模型逻辑
    model_name = cfg.LLM_MODEL_DIR  # 主模型路径（如THUDM/chatglm3-6b）

    tokenizer: AutoTokenizer = None   # 分词器
    model: AutoModel = None   # 模型本体
    config: AutoConfig = None   # 模型配置
    isClassify = False
    isKeywords = False
    isNL2SQL = False

    # 通过传入微调权重类型来加载不同的权重进行工作
    def __init__(self, ptuning_type: PtuningType):  # 接收p-tuning类型参数
        super().__init__()
        check_point_path = ""  # 微调权重路径占位
        # 载入Tokenizer      所有任务共享
        ...
        if ptuning_type is PtuningType.Classify or ptuning_type is PtuningType.NL2SQL or ptuning_type is PtuningType.Keywords:
            ...
            # 如果是分类类型，加载长度为2048的NL2SQL训练权重
                ...
                self.isNL2SQL = True
            # 装载对应路径的权重
            prefix_state_dict = torch.load(check_point_path)  # 加载权重文件

            # 键名转换（适配模型结构）
            new_prefix_state_dict = {}  # 创建空字典用于存储调整后的权重键值对

            # 遍历原始权重条目
            for k, v in prefix_state_dict.items():
                # 仅处理以 transformer.prefix_encoder. 开头的键
                if k.startswith("transformer.prefix_encoder."):
                    # 移除前缀 transformer.prefix_encoder.，保留子模块路径（如 embeddings.weight）
                    new_prefix_state_dict[k[len("transformer.prefix_encoder."):]] = v
            # 加载适配后的权重
            # prefix_encoder 是 P-tuning 的核心组件，负责生成可训练的前缀向量
            # 通过 load_state_dict 将调整后的权重注入模型
            self.model.transformer.prefix_encoder.load_state_dict(new_prefix_state_dict)
            # 将 prefix_encoder 转为 FP32
    ...
    @property # 通过 @property 装饰器定义只读属性，允许以字段形式访问（self._get_nl2sql_prompt）
